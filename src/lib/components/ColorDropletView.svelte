<script lang="ts">
  ...
  let color = $derived(getFlagColor(colorName));
  // svelte-ignore state_referenced_locally
  const draggable = createDraggable({ id: colorName });
</script>

<div
  class="flex h-16 w-16 cursor-grab flex-col items-center justify-center border-2 border-white bg-blue-700/60 p-2"
